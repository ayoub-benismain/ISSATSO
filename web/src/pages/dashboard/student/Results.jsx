...
const resultsData = {
  "Semester 1": [
    { name: "Mathematics", ds: 16, tp: 18, exam: 14, coefficient: 2 },
    { name: "Physique", ds: 12, tp: 15, exam: 13, coefficient: 1.5 },
    { name: "Algorithm", ds: 17, tp: 19, exam: 16, coefficient: 3 },
    { name: "English", ds: 14, tp: 15, exam: 13, coefficient: 1 },
  ],
  "Semester 2": [
    { name: "Mathematics", ds: 15, tp: 17, exam: 16, coefficient: 2 },
    { name: "Physique", ds: 14, tp: 14, exam: 15, coefficient: 1.5 },
    { name: "Algorithm", ds: 18, tp: 17, exam: 19, coefficient: 3 },
    { name: "English", ds: 16, tp: 17, exam: 14, coefficient: 1 },
  ],
};

// Function to generate random marks between 8 and 20
const randomMark = () => Math.floor(Math.random() * 13) + 8;

export default function ResultsStudent() {
  const [selectedSemester, setSelectedSemester] = useState("Semester 1");
  const [dropdownOpen, setDropdownOpen] = useState(false);

  let subjects = resultsData[selectedSemester];
  ...
  return (
    <div className="p-4 lg:p-6 lg:w-full min-h-screen">
              ...
              >
                {Object.keys(resultsData).map((sem) => (
                  <li
                    ...
                    onClick={() => { setSelectedSemester(sem); setDropdownOpen(false); }}
                  >
      ...
      <div className="overflow-x-auto">
        <div className="min-w-screen lg:min-w-full">
          <div className="flex bg-gray-100 text-gray-600 uppercase text-sm font-semibold rounded-t-xl">
            <div className="flex-1 px-2 py-3 flex items-center gap-2"><BookOpen size={14}/> Subject</div>
            <div className="w-14 px-2 py-3 text-center"><Calendar size={14}/> DS</div>
            <div className="w-14 px-2 py-3 text-center"><Calendar size={14}/> TP</div>
            <div className="w-14 px-2 py-3 text-center"><Calendar size={14}/> Exam</div>
            <div className="w-14 px-2 py-3 text-center"><BarChartIcon size={14}/> Final</div>
            <div className="w-14 px-2 py-3 text-center"><Hash size={14}/> Coef</div>
          </div>
          ...
          {subjects.map((sub, idx) => (
            ...
            >
              ...
              <div className="w-14 px-2 py-3 text-center">
                <span className="px-2 py-1 rounded-full text-white bg-blue-500 text-sm">
                  {((sub.ds + sub.tp + sub.exam) / 3).toFixed(1)}
                </span>
              </div>
      ...
      >
        ...
        <span className="text-2xl lg:text-3xl font-extrabold ml-4">?</span>
      </motion.div>
